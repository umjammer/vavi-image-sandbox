...
public abstract class BasicEnlarger<T extends Primitive<T>> {
    ...
    private static final long smallToBigMargin = Constants.blockLen + 40; // need values<0,>size in smallToBigTabs

    private BasicArray<T> srcBlock, dstBlock; // srcBlock contains additional margins of borderPixels
    ...
    private int outputWidth, outputHeight; // size of resulting image

    private int offsetX, offsetY; // pos of cliprect within image ( generate black margins )

    private int clipX0, clipX1, clipY0, clipY1; // clip-rect

    private boolean onlyShrinking; // if scaleF < 1.0: don't allocate blocks&tabs, only call ShrinkClip()
    ...
    private float[] bigPixelCenterW = new float[5 * 5]; // increased weight near pixel-center dep. of d2L
    ...
    public BasicEnlarger(final EnlargeFormat format, final EnlargeParameter param) {
        ...
        invTab[0] = 1000000.0f;
        for (a = 1; a < Constants.invTabLen; a++)
    ...
    protected abstract void readSrcPixel(int srcX, int srcY, T dstP);
    ...
    protected void readSrcBlock() {
        ...
        while (sy <= 0 && y < blockSizeY) {
            ...
            // while dst outside: write src-edge-pixel
            while (x < blockSizeX) {
                readSrcPixel(srcSizeX - 1, 0, dst[dstP]);
                dstP++;
                x++;
                sx++;
            }
        ...
        while (sy < srcSizeY - 1 && y < blockSizeY) {
            ...
            // while dst outside: write src-border-pixel
            while (x < blockSizeX) {
                readSrcPixel(srcSizeX - 1, sy, dst[dstP]);
                dstP++;
                x++;
                sx++;
            }
        ...
        while (y < blockSizeY) {
            ...
            // while dst outside: write src-edge-pixel
            while (sx < 0 && x < blockSizeX) {
                readSrcPixel(0, srcSizeY - 1, dst[dstP]);
                dstP++;
                x++;
                sx++;
            }
            ...
            while (sx < srcSizeX - 1 && x < blockSizeX) {
                readSrcPixel(sx, srcSizeY - 1, dst[dstP]);
                dstP++;
                x++;
                sx++;
            }
            // while dst outside: write src-edge-pixel
            while (x < blockSizeX) {
                readSrcPixel(srcSizeX - 1, srcSizeY - 1, dst[dstP]);
                dstP++;
                x++;
                sx++;
            }
