...
package vavix.awt.image.quantize;
 ...
 */
public class NeuralNetQuantizerOp implements BufferedImageOp {

    /** */
    private int colors;

    /** */
    public NeuralNetQuantizerOp(int colors) {
        this.colors = colors;
    }
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dest) {
        ...
        init(src.getRaster().getPixels(0, 0, src.getWidth(), src.getHeight(), (int[]) null), src.getWidth(), src.getHeight(), colors);
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                dest.setRGB(x, y, convert(src.getRGB(x, y)));
            }
        }
        return dest;
    }
    ...
    /** */
    private native void init(int[] src, int width, int height, int colors);

    /** */
    private native int convert(int rgb);
